(function (g, e) {
  ...
  g.ajaxSetup({
    ...
    encodePath: !0,
    hook: !0,
    beforeSend: function (h, c) {
      ("undefined" !== typeof G_IS_HOOKED && G_IS_HOOKED(c.url)) ||
        (c.externalize && (c.url = k.externalize(c.url)),
        c.encodePath && (c.url = k.encodePathOfURI(c.url)));
      c.hook &&
        (h = k.getXhrHook(c.url, c.type, c.data)) &&
        ((c.url = h.url),
        h.params &&
          ("GET" === c.type.toUpperCase()
            ? (c.url += "?" + g.param(h.params))
            : (c.data = g.param(h.params))));
    },
    statusCode: {
      403: function (h) {
        "Authentication Failed" === h.getResponseHeader("X-Reason") &&
          k.handleLoginRedirect();
      },
    },
  });
  g.ajaxSettings.traditional = !0;
})(jQuery, this);
(function (g) {
  window.Granite.csrf || (window.Granite.csrf = g(window.Granite.HTTP));
})(function (g) {
  function e() {
    this._handler = [];
  }
  ...
  function c() {
    ...
    b.onreadystatechange = function () {
      if (4 === b.readyState)
        try {
          (f = JSON.parse(b.responseText).token), a.resolve(f);
        } catch (d) {
          h(d), a.reject(b.responseText);
        }
    };
    b.open("GET", n, !0);
    b.send();
  ...
  function p() {
    ...
    a.send();
    try {
      return (f = JSON.parse(a.responseText).token);
    } catch (b) {
      h(b);
    }
  }
  ...
  e.prototype = {
    ...
    _execute: function (a, b) {
      ...
      this.then = function (u, v) {
        ("resolve" === a ? u : v).apply(window, b);
      };
      this._handler = null;
    },
  };
  var n = g.externalize("/libs/granite/csrf/token.json"),
    m,
    f;
  r(document);
  var w = XMLHttpRequest.prototype.open;
  XMLHttpRequest.prototype.open = function (a, b, d) {
    "get" !== a.toLowerCase() && k(b) && ((this._csrf = !0), (this._async = d));
    return w.apply(this, arguments);
  };
  var l = XMLHttpRequest.prototype.send;
  XMLHttpRequest.prototype.send = function () {
    if (this._csrf)
      if (f) this.setRequestHeader("CSRF-Token", f), l.apply(this, arguments);
      else if (!1 === this._async)
        p(),
          f && this.setRequestHeader("CSRF-Token", f),
        ...
        m.then(
          function (d) {
            a.setRequestHeader("CSRF-Token", d);
            l.apply(a, b);
          },
          function () {
            l.apply(a, b);
  ...
  if (window.Node) {
    var y = Node.prototype.appendChild;
    Node.prototype.appendChild = function () {
      ...
      if ("IFRAME" === a.nodeName)
        try {
          a.contentWindow &&
            !a._csrf &&
            ((a._csrf = !0), r(a.contentWindow.document));
        } catch (b) {
          a.src &&
            a.src.length &&
            k(a.src) &&
